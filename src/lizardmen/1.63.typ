...
#book-meta(
  ...
  version: "1.63",
  layout: "army",
  cover: "covers/lizardmen/1.63.png",
  align: "order",
)
...
#cover(
  ...
  art: "/assets/covers/lizardmen/1.63.png",
)
...
Remains in play. _Entomb_ is a *hex* with a range of 24" that can be cast on an unengaged unit. While the spell is in effect, the target cannot move, shoot or be damaged in any way (except by spells from the Lore of Geomancy) and is treated as impassable terrain for all purposes.
...
_Gravitic Redirection_ is a *direct damage* spell with a range of 12". All models in the target unit suffers a Wound which Ignores Armour Saves on the roll of a 4+.

#namecost("ASSAULT OF STONE", "")
#namecost("Level 4 Cast on 12+", "")

_Assault of Stone_ is a *direct damage* spell. Place the small round template anywhere within 24" of the Wizard – it then scatters D3". All models under the template suffer a Strength 5 hit.
...
_The Great Leveller_ is a special type of spell that targets all units (friend and foe, excluding the caster) within 12", even if they are engaged in close combat. All target units suffer 3D6 Strength 5 Hits.
]
...
The Slann Mage-Priest has the Loremaster special rule. This discipline cannot be combined with the Wandering Deliberations discipline.
...
Slann Mage-Priest only. This talisman gives the wearer and any unit they are with a 5+ Ward save against missile attacks.

#namecost("AURA OF QUETZL", "30 points")

This talisman gives the wearer a 4+ Ward save special rule against attacks with Strength 5 or more.
...
One use only. If this model would be removed as a casualty, immediately roll a D6. On a 3+, the last Wound is ignored and the model remains with 1 Wound.

#entry("ARCANE ITEMS")
#namecost("CRYSTALLINE SKULL", "60 points")

Each time the bearer of this item successfully casts a spell, add +1 Power Dice to the pool (to a maximum of 3 per Magic Phase). In addition, once per battle the bearer may choose to destroy this item at the end of any of your Magic phases. If they do so, pick one enemy unit within 12". Roll a number of dice equal to the number of Power dice in the pool. For each roll of a 3+, that unit suffer a Wound that Ignores Armour Saves.
...
Bound Spell, (power level 5). This item contains the _Urannon's Thunderbolt_ spell from the Lore of Heavens.

#namecost("THE TETRAGON OF TECTONIC", "")
#namecost("DISPLACEMENT", "35 points")

Bound Spell (power level 5). The Tetragon of Tectonic Displacement contains the _Tectonic Shift_ spell from the Lore of Geomancy.
...
Bound Spell (power level 5). This item contains a *direct damage* spell with a range of 12" that targets one enemy model, even in a unit. The target model must pass a Toughness test or suffer a Wound which Ignores Armour Saves. If the model takes a Wound from this spell, it must immediately take another Toughness test, losing another Wound if it fails again. This continues until either the model passes a Toughness test or is slain.

#namecost("PRISM OF AMYNTOK", "35 points")

Slann only. Bound spell (power level 5). This item contains a *direct damage* spell. Extend a straight line, 12" in length, directly away from the caster. Each model in the way (determined using the line template) suffer a Strength 6 Hit with the Flaming Attacks special rule. This has a +1 bonus To Wound against Daemons, Undead and Vampires.
...
Bound Spell, power level 5. This standard contains a *conveyance* spell that may be cast on the unit carrying it. The unit may immediately make a move directly forward using the Random Movement (2D6) special rule.
...
#field("MAGIC", "A Slann Mage-Priest is a Level 4 Wizard who chooses spells from one of the following Lores of Magic:")
...
- *Mage-Priest Palanquin:* The Palanquin gives the Slann Mage-Priest the Strider special rule. Additionally, while they have the troop type Monstrous Infantry, they are never considered to be ‘models on foot' for the purposes of spells, magic items and special rules.
- *Shield of the Old Ones:* A model with this special rule has a 4+ Ward Save.
...
#field("BASE SIZE", "20x20 or 25x25")
#field("EQUIPMENT", "Hand weapon")
#field("MAGIC", "A Skink High Priest is a Level 3 Wizard who chooses spells from one of the following Lores of Magic:")
...
#field("SPECIAL RULES", "Cold-blooded, Predatory Fighter, Natural Armour (3+)")
#field("OPTIONS", "")
...
#field("BASE SIZE", "20x20 or 25x25")
#field("EQUIPMENT", "Hand weapon")
#field("MAGIC", "A Skink Priest is a Level 1 Wizard who chooses spells from one of the following Lores of Magic:")
...
#compact-entry("TEMPLE GUARD PALANQUIN")[
#profile(
...
#field("SPECIAL RULES", "Cold-blooded, Predatory Fighter, Stubborn")
]
...
#field("SPECIAL RULES", "Aquatic, Cold-blooded, Fast Cavalry, Jungle Poisons, Natural Armour (6+)")
#field("OPTIONS", "")
...
#field("BASE SIZE", "20x20 or 25x25")
#field("EQUIPMENT", "Hand weapon, blowpipe")
#field("SPECIAL RULES", "Aquatic, Chameleon, Cold-blooded, Loner, Natural Armour (6+), Scouts, Skirmishers")
#field("OPTIONS", "")
...
- *Howdah:* The Dread Saurian is ridden by 6 Skinks (Lizardmen) who are equipped with blowpipes and have the Jungle Poisons and Natural Armour (6+) special rules.
...
- *Tepok's Crystalline Eye:* The Dread Saurian gains a 5+ Ward save which is reduced to a 6+ Ward save against Magical Attacks.
...
- *Mage-lord of Hexoatl:* Lord Mazdamundi has the Loremaster special rule. In addition, he can use one more power dice than normal when casting spells.
...
#field("SPECIAL RULES", "Arcane Might (see Slann-Mage Priests), Cold-blooded, Flammable, Mage-Priest Palanquin, Unbreakable")
...
- *Sacred Spawning of Xhotl:* Kroq-Gar has a 5+ Ward save. If Kroq-Gar is wounded in close combat and does not pass his Ward save, then whoever struck the blow immediately suffers an automatic Strength 5 hit.
...
#field("SPECIAL RULES", "Aquatic, Arcane Vassal, Cold-blooded, Hatred (Skaven), Immunity (Poisoned Attacks), Natural Armour (6+)")
...
- *Minor Shield of the Old Ones:* Tetto'eko has a 5+ Ward Save.
...
- *The Key to the Eternity Chamber:* Enchanted Item. When fighting in a challenge, Chakax has a 5+ Ward save and his opponent gains the Always Strikes Last special rule.
...
- *Ultimate Bodyguard:* Whilst Chakax is part of a combined unit that contains both a Slann and Temple Guard, all models in the combined unit gain the Unbreakable special rule.
...
#field("BASE SIZE", "25x50 or 30x60")
#field("EQUIPMENT", "Light lance, shield")
#field("SPECIAL RULES", "Cold-blooded, Fear, Natural Armour (5+), Poisoned Attacks")
...
- *Tichi-Huichi's Raiders:* Tichi-Huichi must be accompanied by a unit of Horned One Riders chosen from the army list at a cost of 22 points per model. This unit has the Poisoned Attacks special rule. They may not take any Sacred Spawning. The unit must be given a Standard Bearer and Musician. Tichi-Huichi is the unit's Leader and may never choose to leave this unit.
...
#field("SPECIAL RULES", "Aquatic, Chameleon, Cold-blooded, Hatred (Daemons of Chaos), Jungle Poisons, Loner, Natural Armour (6+), Scouts, Sniper")
