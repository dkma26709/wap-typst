// Warriors of Chaos 1.6 - re-typeset from the Warhammer Armies Project PDF.
...
#show: book.with(title: "Warhammer Armies Project — Warriors of Chaos 1.6")
...
#colophon((
  [
    ...
    Warriors of Chaos*, version 1.6 — written and freely distributed by Mathias
    Eliasson.
  ],
  [
...
Ensorcelled Weapons are hand weapons which use the following profile:

#minitable(("Range", "Strength", "Special Rules"), ("Combat", "+1", "Magical Attacks"))
]

#upgrade("DAEMONIC", none)[
Models with this special rule have the Fear and Magical Attacks special rules. In addition, models that are not mounts have a 5+ Ward save which gets reduced to a 6+ Ward save against Magical Attacks.
...
- *Daemon of Nurgle:* A Daemon of Nurgle has the Hatred (Daemons of Tzeentch) special rule, and enemy models in base contact suffer \-1 to their Weapon Skill.

- *Daemon of Slaanesh:* A Daemon of Slaanesh has the Hatred (Daemons of Khorne) and Armour Piercing (1) special rules. Each To Hit roll of 6 generates an additional Attack. These additional Attacks cannot generate further attacks.

- *Daemon of Tzeentch:* A Daemon of Tzeentch has the Hatred (Daemons of Nurgle) special rule and a 6+ Ward save. Wizards with the Daemon of Tzeentch upgrade can also re-roll channelling results of 1.
]
...
In addition, if a model with this special rule kills an enemy character or destroys a non-Expendable unit in either close combat, with spells, or through charging/pursuing, immediately make a 2D6 roll on the Eye of the Gods table. If there is more than one character in the unit at the time of the destruction of the enemy unit through pursuit, each character gets to make a separate roll. Once the result of the roll has been determined, make a note on your army roster – that model now has that reward for the rest of the battle.

A model can have several rewards, and it can even have the same reward multiple times. If, for any reason, a reward cannot be applied, the result is instead treated as a ‘The Eye Opens’ result.

Note that the model itself must cause the last unsaved Wound on an enemy model to claim the kill. If an enemy model suffered multiple Wounds simultaneously, randomise between the sources to determine which caused the final Wound.
]

#upgrade("MARKS OF CHAOS", none)[
Many characters and regiments in the Warriors of Chaos army have, or can purchase, one of the four Marks of Chaos detailed below. A character with a Mark of Chaos cannot join a unit that has a different Mark of Chaos. A character with a Mark of Chaos cannot join a unit that has already been joined by a character that has a different Mark of Chaos.
...
Models with this special rule always re-roll failed Panic tests.

#chartlabel("EYE OF THE GODS TABLE")
#chart((("2D6", "Result"), ("2", "Damned by Chaos: The model is removed from
play as a casualty. If the model is mounted, its
mount is also removed from play.
If you have a spare Chaos Spawn model, you can
place it where the model was. If the model was in
close combat, the Spawn will remain so if
possible. If the model was in a unit before being
turned into a Spawn, place the Spawn within 3\" of
the model's last position, at least 1\" away from
any unit or impassable terrain. It retains none of
the equipment, upgrades, magic items, Chaos
Mutations & Powers, special rules or spells. If the
model had a Mark of Chaos, it now instead has
the Spawn upgrade of the corresponding god. The
new Spawn does not generate any new Victory
Points.
If you do not have a spare Chaos Spawn model, or
if it cannot be placed on the board according to
the aforementioned restrictions, then no Chaos
Spawn model is placed."), ("3", "Aura of Chaos: The model gains the Ward save
(6+) special rule."), ("4", "Unholy Resilience: The model gains +1
Toughness."), ("5", "Iron Skin: The model gains the Natural Armour
(6+) special rule."), ("6", "Murderous Mutation: The model gains +1
Weapon Skill."), ("7", "The Eye Opens: The model can re-roll one failed
roll To Hit, roll To Wound or saving throw until
the end of his next turn."), ("8", "Unearthly Reflexes: The model gains +1
Initiative.")))
#chart((("2D6", "Result"), ("9", "Dark Fury: The model gains +1 Attack."), ("10", "Slaughterer's Strength: The model gains +1
Strength."), ("11", "Command of the Gods: The model gains +1
Leadership."), ("12", "Dark Apotheosis: The model is replaced with a
Daemon Prince (if you have a spare Daemon
Prince model) where the model was. Any Wounds
lost during the game before becoming a Daemon
Prince are not carried over. If the model is
mounted, its mount is also removed from play. If
the model was in close combat, the Daemon
Prince will remain so if possible. If the model was
in a unit before being turned into a Daemon
Prince, place the Daemon Prince within 3\" of the
model's last position, at least 1\" away from any
unit or impassable terrain. This Daemon Prince
will retain any Chaos Mutations & Powers, Magic
Items and any other rewards from this table
gained (if it had any). If the model had a Mark of
Chaos, it now instead has the Daemon upgrade of
the corresponding god. Any mundane equipment
is removed. If the model was your Army General
or Battle Standard Bearer, the Daemon Prince
remains so. If the model was a Wizard, the
Daemon Prince retains his Wizard levels and
knows the same spells as the model did before
turning into a Daemon Prince. The new Daemon
Prince does not generate any additional Victory
Points, use the model's original cost.
If you do not have a spare Daemon Prince model,
or if it cannot be placed on the board according to
the aforementioned restrictions, then no Daemon
Prince is placed and the model is removed from
play. No Victory Points are awarded in this case.")))
]
]
#upgrade-chapter("MUTATIONS & POWERS")[
*Below are Mutations and Powers that can be bought for your characters. Each can only be chosen once per army (excluding Special Characters).*
...
#upgrade("FEARSOME AURA", 30)[All enemy units within 6" of this character suffer \-1 to their Leadership.]

#upgrade("TERRIFYING APPEARANCE", 25)[The character has the Terror special rule.]

#upgrade("WINGS", 25, only: "Infantry")[The character has the Fly (10) special rule.]

#upgrade("FLAMING BREATH", 25)[The character has a Strength 3 Breath Weapon that has the Flaming Attacks special rule.]

#upgrade("BESTIAL VISAGE", 15)[The character has the Fear special rule.]
...
#upgrade("POISONOUS SLIME", 10)[The character has the Poisoned Attacks and Immunity (Poisoned Attacks) special rules.]

#upgrade("SCALED SKIN", 10)[The character has the Natural Armour (6+) special rule.]

#upgrade("ACID ICHOR", 5)[Whenever this character suffers an unsaved Wound in close combat, the model that inflicted that Hit suffers a Strength 4 hit, which count towards the combat result.]

#upgrade("BURNING BODY", 5)[The character has the Flaming Attacks and Immunity (Flaming Attacks) special rules.]

#upgrade("CLOVEN HOOVES", 5, only: "Model on foot")[The Champion gains +1 Movement. This mutation cannot be combined with Serpent Body.]

#upgrade("HORNS", 5, only: "Model on foot")[The character gets the Impact Hits (1) special rule.]

#upgrade("TENTACLE", 5)[The character may grapple with a single opponent in base contact, forcing them to lose one Attack. However, the character is not allowed to use a shield or any weapon that Requires Two Hands.]
...
#group("POWERS OF KHORNE", note: [_The following powers may only be taken by models with the Mark of Khorne or Daemon of Khorne._])

#upgrade("BLOODCURDLING ROAR", 25)[At the start of the first round of close combat, all enemy units in base contact suffer D6 Strength 2 hits which Ignores Armour saves.]

#upgrade("BLOOD FEVER", 25)[The model will never lose their Frenzy for any reason. In addition, any unit, friend or foe, that is in base contact with them at the start of their Movement phase becomes subject to Frenzy for the rest of that turn. If they already have the Frenzy special rule, they instead gain +1 Attack, but will have to re-roll any successful Berserk Rage roll.]

#upgrade("FURY OF THE BLOOD GOD", 25)[The model gains the Hatred special rule. Any Wizard within 12" of the character suffer -D3 to each of their casting rolls.]
...
#group("POWERS OF NURGLE", note: [_The following powers may only be taken by models with the Mark of Nurgle or Daemon of Nurgle._])
...
#group("POWERS OF SLAANESH", note: [_The following powers may only be taken by models with the Mark of Slaanesh or Daemon of Slaanesh._])
...
#upgrade("WORD OF AGONY", 30)[Once per game, at the beginning of the Close Combat phase (before challenges are issued), the bearer of the Word of Agony can choose a model in base contact. That model takes D6 Strength 4 hits which Ignores Armour saves.]

#upgrade("SERPENT BODY", 15, only: "Model on foot")[The Champion gains +2 Movement, +1 Initiative and the Swiftstride special rule. This mutation cannot be combined with Cloven Hooves.]

#upgrade("SOPORIFIC MUSK", 15)[When a unit flees from a character with this gift or the unit they are with, the fleeing unit rolls an extra D6 and discards the highest dice roll.]
...
#group("POWERS OF TZEENTCH", note: [_The following powers may only be taken by models with the Mark of Tzeentch or Daemon of Tzeentch._])
...
#magic-weapon("CHAOS DAEMONSWORD", 50)[A model with the Daemonsword adds D3 to their Strength and has an extra D3 Attacks, to a maximum of 10 (roll separately for these values at the start of each round). However, every To Hit roll of a 1 made by this character in close combat is resolved against themself; the character can never re-roll a To Hit roll of a 1.]

#magic-weapon("RENDING SWORD", 45)[All attacks with the Rending Sword may re-roll failed To Wound rolls and have the Multiple Wounds (D3) special rule.]

#magic-weapon("HELLFIRE SWORD", 35)[Close combat attacks made with the Hellfire Sword have the Flaming Attacks and Ignores Armour Saves special rules. After all close combat blows have been struck, roll a D6 for every foe slain by the Hellfire Sword – on the roll of a 6, the slain foe’s body explodes, inflicting an additional D6 Strength 4 hits on the enemy unit with the Flaming Attacks special rule. Unsaved Wounds inflicted in this way count towards the combat result. At the end of each of the wielder’s turns, roll a D6; on the roll of a 1, they suffer one Wound with the Ignores Armour Saves special rule.]

#magic-weapon("VAMPIRIC AXE", 30)[Great weapon. For each unsaved Wound the wielder inflicts with this weapon, they regain +1 Wound up to their starting value.]
...
#magic-weapon("THE FATHER OF BLADES", 20)[All To Hit rolls of 1 directed against the wielder of the Father of Blades in close combat instead hit the attacking model.]
...
#magic-weapon("SOUL CLEAVER", 5)[Any model that suffers an unsaved Wound from the Soul Cleaver must pass a Toughness test or suffer an additional wound with no saves allowed. This additional wound does not cause an additional Toughness test.]
]

#magic-item-section("armour")[
#magic-armour("VOID ARMOUR", 50)[Model on foot only. Heavy armour. The wearer gains the Ethereal special rule. However, they may take no other Magic Items.]

#magic-armour("ARMOUR OF MORRSLIEB", 45)[Heavy armour. The Armour of Morrslieb gives the wearer a 4+ Ward Save against non-Magical Attacks.]

#magic-armour("CRIMSON ARMOUR OF DARGAN", 35)[Heavy armour. The wearer of the Crimson Armour gains Immunity (Killing Blow/Multiple Wounds) against close combat attacks and a Ward save (6+).]

#magic-armour("HELM OF THE OPPRESSOR", 35)[This item gives the wearer a 6+ armour save. All enemy units within 6" of the wearer of this helmet suffer \-1 to their Leadership.]

#magic-armour("IDOLATROUS PLACKART", 35)[Heavy armour. The wearer of this armour have a 5+ Ward save which gets reduced to a 6+ Ward save against Magical Attacks.]

#magic-armour("ARMOUR OF THE DAMNED", 30)[Heavy armour. Enemy models must re-roll successful To Hit rolls in close combat against the wearer of this armour.]

#magic-armour("NULL PLATE", 30)[Heavy armour. All close combat and missile attacks targeting the wearer of this armour suffer \-1 To Hit.]

#magic-armour("BRONZE ARMOUR OF ZHRAKK", 25)[Heavy armour. The Bronze Armour of Zhrakk gives the wearer the Animated Construct special rule (though they may still march). However, no other model may use the model's Leadership.]

#magic-armour("SKINHIDDEN PLATE", 25)[Heavy armour. The Skinhidden Plate gives the wearer the Natural Armour (5+) special rule.]

#magic-armour("DESECRATOR GAUNTLETS", 20)[This item gives the wearer a 6+ armour save. All enemy Wizards within 12" of the wearer of these gauntlets suffer \-2 to their casting results. In addition, the wearer gains +1 To Wound against Wizards in close combat.]
...
#magic-armour("ARMOUR OF TORTURED SOULS", 15)[Heavy armour. The Armour of Tortured Souls gives the wearer +1 Toughness against non-Magical Attacks.]

#magic-armour("FUSED ARMOUR", 15)[Heavy armour. The wearer of this armour automatically passes all Characteristic tests (except Leadership tests).]

#magic-armour("WEIRD PLATE", 15)[Heavy armour. All models attempting to strike the wearer of this armour suffer \-1 Attack (to a minimum of 1).]

#magic-armour("HELM OF ELDRITCH COMMAND", 10)[Wizard only. This item gives the wearer a 6+ armour save. Whenever the wearer successfully dispels an enemy spell (except Summoning or Innate Bound Spells), they may attempt to cast that spell themselves (limited by their Wizard level as normal) in their next Magic phase.]

#magic-armour("SHIELD OF TEETH", 5)[Shield. If the bearer of this shield makes a successful Parry save, they immediately get to make an additional Attack.]
]
...
#enchanted-item("ROD OF TORMENT", 35)[Bound Spell, Power Level 5. The Rod of Torment contains a *magic missile* with a range of 18" and causes 2D6 Strength 3 hits.]
...
#enchanted-item("STAR OF CHAOS", 15)[Ungorr may re-roll one failed To Hit, To Wound, Armour or Ward Save roll in each phase.]

#enchanted-item("CHALICE OF CHAOS", 10)[One use only. The bearer of the Chalice of Chaos may drink from it at the start of any phase. If he chooses to do so, roll a D6 and consult the table below.

#chart((("D6", "Result:"), ("1", "Unworthy Fool: The imbiber immediately
suffers a Wound, with no saves of any kind
allowed."), ("2", "Inhuman Speed: The imbiber gains the
Always Strikes First special rule until the end
of the turn."), ("3", "Regenerating Flesh: The imbiber gains the
Regeneration (4+) special rule until the end of
the turn."), ("4", "Dark Fortune: The imbiber gains the Ward
save (5+) special rule until the end of the turn."), ("5", "Daemonic Strength: The imbiber gains the
Killing Blow special rule until the end of the
turn."), ("6", "Final Transformation: If the imbiber is
...
#enchanted-item("PENDANT OF DAMNATION", 10)[Infantry or Cavalry only. The bearer of this item gains +1 Attack for every Wound they suffer during the game. If their lost Wounds are restored, this bonus is lost.]

#enchanted-item("FAVOUR OF THE GODS", 5)[After rolling on the Eye of the Gods table the bearer may add or subtract one from his roll. This may not affect a result of 2 or 12.]
]
...
#magic-standard("BANNER OF WRATH", 25)[Bound Spell, power level 4. The Banner of Wrath contains a *magic missile* with a range of 24". If cast, it causes D6 Strength 4 hits with the Lightning Attacks special rule.]
]
...
#magic-item-section("weapon", first: true)[
#magic-weapon("BANE SPEAR", 30)[Spear/light lance. This weapon gives +1 Strength for each enemy model in base contact with the wielder.]

#magic-weapon("BERSERKER SWORD", 30)[Model on foot only. The Berserker Sword gives the wielder +1 Attack for every enemy model in base contact. In a challenge, it only confers +1 Attack.]
...
#magic-weapon("BLADE OF BLOOD", 30)[For every unsaved Wound caused by the Blade of Blood, the wielder may take a Strength test. If passed, the wielder regains 1 Wound previously lost during the game. However, if the test is failed, the wielder loses 1 Wound with no saves allowed instead.]
...
#magic-item-section("armour")[
#magic-armour("SPELLEATER SHIELD", 40)[Shield. The Spelleater Shield gives the bearer Magic Resistance (2). In addition, roll a D6 each time a spell is cast at the bearer of the Spelleater Shield and is dispelled. On the roll of a 4+, the spell is destroyed and the enemy Wizard may not cast it for the rest of the battle.]

#magic-armour("THE BLOOD-FORGED ARMOUR", 30)[Heavy armour. The wearer of this item cannot have their armour reduced below 4+.]
...
#talisman("THE BRAZEN RUNE", 15)[The wearer of this item gains a 2+ Ward save against spells.]
]

#magic-item-section("enchanted")[
#enchanted-item("MARK OF THE DESTROYER", 50)[The wielder doubles their amount of Attacks (to a maximum of 10). However, if the wearer fails to kill at least one model each close combat phase in which they are part of, treat this as having rolled the Damned by Chaos result on the Eye of the Gods chart.]
...
#enchanted-item("THE BLACK TONGUE", 10)[One use only. The Black Tongue can be used whenever an enemy Wizard fails to cast a spell. When used, the spell counts as having been Miscast. The model bearing the Black Tongue then suffers a wound with no saves allowed.]
]
...
#magic-weapon("GLAIVE OF PUTREFACTION", 15)[Polearm. Every unsaved Wound caused by the Glaive of Putrefaction against Characters, Monstrous Creatures or Monsters makes the victim suffer \-1 to their Strength and Toughness for the remainder of the game.]
]
...
#magic-armour("THE SHIELD OF GROWTHS", 10)[Shield. The bearer of this shield may re-roll failed armour saves if the Strength of the attack is equal to or higher than the bearer's modified armour save (for example, if the bearer is Hit by a Strength 4 Attack and their modified armour save is 4+).]
]

#magic-item-section("talisman")[
#talisman("SPORE CENSER", 25)[All missile attacks targeting the wearer or the unit they are with suffer \-1 To Hit.]

#talisman("THE FOETID SHROUD", 25)[All close combat attacks targeting the wearer of this item suffer \-1 To Hit.]
]

#magic-item-section("arcane")[
#arcane-item("ROD OF CORRUPTION", 30)[Bound Spell, Power Level 3. The Rod of Corruption contains a *direct damage* spell with a range of 24". The target unit suffers D6 Strength 2 Hits with the Ignores Armour Saves special rule.]
]
...
#enchanted-item("THE CARRION DIRGE", 15)[One use only. This item may be used at the start of any of your turns. Once used, all enemy units within 12" suffer \-2 to their Leadership for the duration of the turn.]

#enchanted-item("THE FECUND FLASK", 10)[One use only. This item can be used at the start of any of your turns. Roll a D6, on a 2+, the model immediately recovers all Wounds (ignoring any additional Wounds from mounts) up to their starting value. On the roll of a 1, treat this as the model having rolled the Damned by Chaos result on the Eye of the Gods chart.]
]
...
#magic-standard("ERODING ICON", 25)[Mark of Nurgle only. All enemy units in base contact with this standard counts the armour save modifier from their Strength or Armour Piercing attacks as being 1 point worse than normal.]
]
...
#magic-weapon("WHIP OF PLEASURE", 30)[Mark of Slaanesh only. The Whip of Pleasure gives the wielder +1 Attack. In addition, the wielder gains the Always Strikes First special rule in the first round of close combat.]

#magic-weapon("WHIP OF SUBVERSION", 25)[Mark of Slaanesh only. If an enemy Character, Monstrous Creature or Monster takes an unsaved wound from the Whip of Subversion and that model has not yet made its attacks for that round, then you may force it to allocate its attacks upon itself or other models from its own side.]

#magic-weapon("BLADE OF ECSTASY", 20)[Mark of Slaanesh only. Each time the Blade of Ecstasy inflicts an unsaved wound on a model, that model must pass a Strength test or be removed from the game.]
...
#magic-item-section("talisman")[
#talisman("JEWEL OF DENIAL", 35)[The bearer of this item gains a 5+ Ward save. In addition, each time they make a successful Ward save in close combat, they gain +1 combat resolution bonus.]
...
#magic-item-section("arcane")[
#arcane-item("SCEPTRE OF DOMINATION", 35)[Bound Spell, Power Level 3. The Sceptre of Domination contains the _Pavane of Slaanesh_ spell from the Lore of Slaanesh.]
]
...
#magic-item-section("armour")[
#magic-armour("PARADOXICAL SHIELD", 10)[Shield. This shield gives the bearer the Parry (5+) special rule even while mount. However, the bearer must re-roll successful armour saves.]
]

#magic-item-section("talisman")[
#talisman("GOLDEN EYE OF TZEENTCH", 30)[The bearer gains a 3+ Ward save against all missile attacks.]

#talisman("CRYSTAL PENDANT", 25)[Infantry or Cavalry only. The bearer of this item may re-roll failed Ward saves.]
]
...
#arcane-item("SOULDRAUGHT", 30)[One use only. This item may be used at the start of any Magic phase. For the duration of this Magic phase, roll an additional D6 when casting or dispelling spells and discard the lowest result.]
...
#arcane-item("MIRROR OF KNOWLEDGE", 15)[The bearer can pick any enemy unit within 24" at the beginning of each of his Magic phases. The opponent must reveal everything that is not apparent about the unit, such as Magic Items and Hidden units.]

#arcane-item("NEXUS STAFF", 10)[Wizard only. For each unsaved Wound caused by the wielder in close combat, they gain a +1 casting bonus in their next Magic phase.]
]

#magic-item-section("enchanted")[
#enchanted-item("WARP MIRROR", 50)[All bonuses that are not from Magic Items or spells (such as Charge Bonus, Mighty Blow, Devastating Charge, Impact Hits etc.) that occur in the first round of close combat are given to the opposite side for both the bearer of this item and any unit they are with as well as any enemy unit in base contact with the bearer.]
...
#enchanted-item("TIMESLIP PENDANT", 20)[One use only. This item may be used after the wearer has made all their normal close combat attacks. When used, they may fight an additional time.]

#enchanted-item("DAEMONHEART", 25)[One use only. This item may be used at the start of any round of close combat. Once used, the wielder gains +2 Strength and +2 Attacks. Roll a D6 after resolving all attacks; on the roll of 1, the bearer suffer a Wound which Ignores Armour Saves.]
]
...
#unit("CHAOS LORD",
  first: true,
  profiles: (
    (name: "Chaos Lord", m: 4, ws: 8, bs: 3, s: 5, t: 5, w: 3, i: 7, a: 5, ld: 9, points: 185),
  ),
...
- May be mounted on one of the following:
  - Chaos Steed +24 points
  - Palanquin of Nurgle (Mark of Nurgle only) +24 points
  - Steed of Slaanesh (Mark of Slaanesh only) +24 points
  - Daemonic Steed +36 points
  - Rot Fly of Nurgle (Mark of Nurgle only) +39 points
  - Disc of Tzeentch (Mark of Tzeentch only) +42 points
  - Serpent of Slaanesh (Mark of Slaanesh only) +48 points
  - Juggernaut of Khorne (Mark of Khorne only) +54 points
  - Rot Beast of Nurgle (Mark of Nurgle only) +57 points
  - Chaos Chariot (replacing one of the crew) +110 points
  - Gorebeast Chariot (replacing one of the crew) +130 points
  - Karkadrak +150 points
  - Manticore +150 points
  - Chaos Dragon +330 points
- May take Mutations & Powers and/or Magic Items up to a total of 100 points
  ],
...
#unit("CHAOS SORCERER LORD",
  profiles: (
    (name: "Chaos Sorcerer Lord", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 3, i: 4, a: 3, ld: 8, points: 215),
  ),
  troop-type: "Infantry (Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon",
  magic: "A Chaos Sorcerer Lord is a Level 3 Wizard who uses spells from the Lore of Fire, Metal, Shadow, Death or Chaos. A Chaos Sorcerer Lord with the Mark of Tzeentch, Nurgle or Slaanesh must use the Lore of Tzeentch, Nurgle, or Slaanesh, respectively.",
  options: [
...
#unit("DAEMON PRINCE",
  ...
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon",
  magic: "A Daemon Prince that is a Wizard uses spells from the Lore of Fire, Metal, Shadow, Death or Chaos. A Daemon Prince with the Mark of Tzeentch, Nurgle or Slaanesh must use the Lore of Tzeentch, Nurgle, or Slaanesh, respectively.",
  options: [
- May take up to four Wizard Levels (except Daemon Princes of Khorne) +35 points/level
- May choose one of the following:
...
- May choose one of the following:
  - Light armour +15 points
  - Medium armour +30 points
...
#unit("EXALTED HERO",
  first: true,
  profiles: (
    (name: "Exalted Hero", m: 4, ws: 7, bs: 3, s: 5, t: 4, w: 2, i: 6, a: 4, ld: 8, points: 105),
  ),
  troop-type: "Infantry (Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon, medium armour",
  options: [
- One Exalted Hero may carry the Battle Standard +25 points
- May choose one of the following:
  - Mark of Khorne +15 points
  - Mark of Slaanesh +15 points
  - Mark of Nurgle +20 points
...
- May take heavy armour +8 points
- May take a shield +2 points
- May be mounted on one of the following:
  - Chaos Steed +16 points
  - Palanquin of Nurgle (Mark of Nurgle only) +16 points
  - Steed of Slaanesh (Mark of Slaanesh only) +16 points
  - Daemonic Steed +24 points
  - Rot Fly of Nurgle (Mark of Nurgle only) +26 points
  - Disc of Tzeentch (Mark of Tzeentch only) +28 points
  - Serpent of Slaanesh (Mark of Slaanesh only) +32 points
  - Juggernaut of Khorne (Mark of Khorne only) +36 points
  - Rot Beast of Nurgle (Mark of Nurgle only) +38 points
  - Chaos Chariot (replacing one of the crew) +110 points
  - Gorebeast Chariot (replacing one of the crew) +130 points
- May take Mutations & Powers and/or Magic Items up to a total of 50 points
  ],
  special-rules: "Chaos Armour, Eye of the Gods, Will of Chaos",
  upgrades: (
...
#unit("CHAOS SORCERER",
  profiles: (
    (name: "Chaos Sorcerer", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 2, i: 4, a: 2, ld: 8, points: 100),
  ),
  troop-type: "Infantry (Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon",
  magic: "A Chaos Sorcerer is a Level 1 Wizard who uses spells from the Lore of Fire, Metal, Shadow, Death or Chaos.",
  magic-body: [
A Chaos Sorcerer with the Mark of Tzeentch, Nurgle or Slaanesh must use the Lore of Tzeentch, Nurgle, or Slaanesh, respectively.
  ],
  options: [
...
- May choose one of the following:
  ...
  - Mark of Tzeentch +20 points
- May choose one of the following:
  - Light armour +6 points
  - Medium armour +12 points
  - Heavy armour +20 points
- May be mounted on one of the following:
  - Chaos Steed +16 points
  - Palanquin of Nurgle (Mark of Nurgle only) +16 points
  - Steed of Slaanesh (Mark of Slaanesh only) +16 points
...
- May be mounted on one of the following:
  - Warhorse +14 points
  - Marauder Chariot (replacing one of the crew) +110 points
- May take Mutations & Powers and/or Magic Items up to a total of 50 points
  ],
...
#unit("CHAOS STEED",
  profiles: (
    (name: "Chaos Steed", m: 8, ws: 3, bs: 0, s: 4, t: 3, w: 1, i: 3, a: 1, ld: 5, points: "-"),
  ),
  troop-type: "War Beast (Animal)",
  base-size: "25x50 or 30x60",
  options: [
...
#unit("DAEMONIC MOUNT",
  profiles: (
    (name: "Daemonic Mount", m: 8, ws: 4, bs: 0, s: 5, t: 4, w: 3, i: 3, a: 2, ld: 8, points: "-"),
  ),
...
#unit("CHAOS WARRIORS",
  first: true,
  profiles: (
    (name: "Chaos Warrior", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 1, i: 4, a: 2, ld: 8, points: 14),
  ),
  unit-size: "10+",
...
- May take shields (if armed with hand weapon or polearms) +2 points/model
- May upgrade one Chaos Warrior to a Leader +5 points
- May upgrade one Chaos Warrior to a Musician +5 points
- May upgrade one Chaos Warrior to a Standard Bearer +10 points
...
- May take shields (if armed with hand weapon or spears) +1 point/model
...
#unit("FORSAKEN",
  profiles: (
    (name: "Forsaken", m: 6, ws: 4, bs: 0, s: 4, t: 4, w: 1, i: 4, a: "*", ld: 8, points: 15),
  ),
  unit-size: "10+",
  troop-type: "Infantry (Human)",
  base-size: "25x25 or 30x30",
  ...
  options: [
- May choose one of the following:
  - Forsaken of Khorne +1 point/model
  - Forsaken of Slaanesh +1 point/model
  - Forsaken of Tzeentch +1 point/model
  - Forsaken of Nurgle +1 point/model
  ],
  special-rules: "Frenzy, Immunity (Psychology), Random Attacks (D3).",
  special-rules-body: [
...
#chart((("D6", "Result"), ("1", "Slug Brains: All Forsaken in the unit is subject to the Always Strikes Last special rule."), ("2", "Razor Talons: All Forsaken in the unit have the Armour Piercing (1) special rule."), ("3", "Lashing Tentacles: All Forsaken in the unit gain +2 to their Initiative."), ("4", "Venomous Fangs: All Forsaken in the unit have the Poisoned Attacks special rule."), ("5", "Healing Flesh: All Forsaken in the unit have the Regeneration (5+) special rule."), ("6", "Decapitating Claws: All Forsaken in the unit have the Killing Blow special rule.")))
  ],
  upgrades: (
    rule("Forsaken of Khorne")[Forsaken of Khorne have the Hatred special rule.],
    rule("Forsaken of Nurgle")[Forsaken of Nurgle have the Fear special rule.],
    rule("Forsaken of Slaanesh")[Forsaken of Slaanesh have the Swiftstride special rule.],
    rule("Forsaken of Tzeentch")[Forsaken of Tzeentch have the Ward save (6+) special rule.],
  ),
...
#unit("WARHOUNDS",
  profiles: (
    (name: "Chaos Warhound", m: 7, ws: 4, bs: 0, s: 3, t: 3, w: 1, i: 3, a: 1, ld: 5, points: 6),
  ),
  unit-size: "5+",
...
#unit("CULTISTS",
  profiles: (
    (name: "Cultist", m: 4, ws: 3, bs: 3, s: 3, t: 3, w: 1, i: 3, a: 1, ld: 6, points: 3),
  ),
  unit-size: "20+",
  troop-type: "Infantry (Human)",
  base-size: "20x20 or 25x25",
...
#unit("CHOSEN",
  first: true,
  profiles: (
    (name: "Chosen", m: 4, ws: 6, bs: 3, s: 4, t: 4, w: 1, i: 5, a: 2, ld: 8, points: 18),
  ),
  unit-size: "10+",
  troop-type: "Infantry (Human)",
  base-size: "25x25 or 30x30",
...
- May choose one of the following:
  ...
  - Flails +3 points/model
  - Polearms +3 points/model
  - Ensorcelled weapons +4 points/model
  - Great weapons +4 points/model
- May take shields (if armed with hand weapon, Ensorcelled weapon or polearms) +2 points/model
...
- May upgrade one Chosen to a Standard Bearer +10 points
  ...
  special-rules-body: (
    rule("Chosen of the Dark Gods")[Chosen have the Eye of the Gods special rule. If the result rolled is Damned by Chaos or Dark Apotheosis, that result is immediately applied to the unit’s Chosen Leader (if the unit does not contain a Chosen Leader, the result is instead treated as a ‘The Eye Opens’ result), otherwise the result is applied to every Chosen model in the unit.],
  ),
  order: ("unit-size", "troop-type", "base-size", "equipment", "options", "special-rules"),
)
...
#unit("CHAOS KNIGHTS",
  profiles: (
    (name: "Chaos Knight", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 1, i: 4, a: 2, ld: 8, points: 30),
    (name: "Chaos Steed", m: 8, ws: 3, bs: 0, s: 4, t: 3, w: 1, i: 3, a: 1, ld: 5, points: ""),
  ),
  unit-size: "5+",
  troop-type: "Cavalry (Human)",
  mount: "Chaos Steed (Animal)",
  base-size: "25x50 or 30x60",
...
- May choose one of the following:
  ...
  - Ensorcelled weapons +4 points/model
- May take Chaos Armour +4 points/model
- May upgrade one Chaos Knight to a Leader +5 points
- May upgrade one Chaos Knight to a Musician +5 points
- May upgrade one Chaos Knight to a Standard Bearer +10 points
  - May take a Magic Standard worth up to 50 points
  ],
  special-rules: "Will of Chaos",
...
#unit("CHAOS CHARIOT",
  profiles: (
    (name: "Chaos Chariot", m: 7, ws: "-", bs: "-", s: 5, t: 5, w: 4, i: "-", a: "-", ld: "-", points: 105),
    (name: "Charioteer", m: "-", ws: 5, bs: 3, s: 4, t: "-", w: "-", i: 4, a: 2, ld: 8, points: ""),
    (name: "Chaos Steed", m: "-", ws: 3, bs: 0, s: 4, t: "-", w: "-", i: 3, a: 1, ld: 5, points: ""),
  ),
  troop-type: "Chariot (Armour Save 6+)",
  crew: "2 Charioteers (Human)",
  drawn-by: "2 Chaos Steed (Animal)",
  base-size: "50x100 or 60x100",
...
- May choose one of the following:
  ...
  - Mark of Tzeentch +20 points
  - Mark of Nurgle +25 points
- May take Chaos Armour +8 points
- May be upgraded to a standard bearer +10 points
  ],
  special-rules: "Will of Chaos",
  order: ("troop-type", "crew", "drawn-by", "base-size", "equipment", "options", "special-rules"),
...
#unit("GOREBEAST CHARIOT",
  profiles: (
    (name: "Gorebeast Chariot", m: 5, ws: "-", bs: "-", s: 5, t: 5, w: 5, i: "-", a: "-", ld: "-", points: 130),
    (name: "Charioteer", m: "-", ws: 5, bs: 3, s: 4, t: "-", w: "-", i: 4, a: 2, ld: 8, points: ""),
    (name: "Gorebeast", m: "-", ws: 4, bs: 0, s: 5, t: "-", w: "-", i: 2, a: 3, ld: 5, points: ""),
  ),
  troop-type: "Chariot (Armour Save 6+)",
  crew: "2 Charioteers (Human)",
  drawn-by: "1 Gorebeast (Chaos Beast)",
  base-size: "50x100 or 60x100",
  equipment: "Polearm, heavy armour, scythes, barding",
  options: [
- May choose one of the following:
  ...
  - Mark of Tzeentch +20 points
  - Mark of Nurgle +25 points
- May take Chaos Armour +10 points
- May be upgraded to a standard bearer +10 points
  ],
  special-rules: "Will of Chaos",
  order: ("troop-type", "crew", "drawn-by", "base-size", "equipment", "options", "special-rules"),
...
#unit("FLAYERKIN",
  profiles: (
    (name: "Flayerkin", m: 4, ws: 4, bs: 3, s: 3, t: 3, w: 1, i: 4, a: 1, ld: 7, points: 6),
  ),
  unit-size: "5+",
  troop-type: "Infantry (Human)",
  base-size: "20x20 or 25x25",
...
- May upgrade one Flayerkin to a Leader +5 points
  ...
  special-rules: "Scouts, Skirmishers, Will of Chaos",
  special-rules-body: (
...
#unit("SKIN WOLVES",
  ...
  options: [
- May choose one of the following:
  - Skin Wolves of Khorne +3 points/model
  - Skin Wolves of Nurgle +3 points/model
  - Skin Wolves of Slaanesh +3 points/model
  - Skin Wolves of Tzeentch +3 points/model
- May upgrade one Skin Wolf to a Leader +5 points
  ],
  special-rules: "Frenzy, Regeneration (5+), Will of Chaos",
  upgrades: (
    rule("Skin Wolves of Khorne")[The model gains the Mighty Blow (1) special rule when charging.],
    rule("Skin Wolves of Nurgle")[The model gains the Poisoned Attacks special rule.],
    rule("Skin Wolves of Slaanesh")[The model gains gain the Always Strikes First special rule.],
    rule("Skin Wolves of Tzeentch")[The model gains the Regeneration (6+) special rule.],
  ),
...
#unit("CHAOS OGRES",
  profiles: (
    (name: "Chaos Ogre", m: 6, ws: 3, bs: 2, s: 4, t: 4, w: 3, i: 2, a: 3, ld: 7, points: 27),
  ),
  unit-size: "3+",
...
- May choose one of the following:
  ...
  - Mark of Slaanesh +6 points/model
- May choose one of the following:
...
- May upgrade one Chaos Ogre to a Leader +5 points
- May upgrade one Chaos Ogre to a Musician +5 points
- May upgrade one Chaos Ogre to a Standard Bearer +10 points
  - May take a Magic Standard worth up to 25 points
  ],
  special-rules: "Loner",
  special-rules-body: (
...
- May choose one of the following:
  ...
  - Mark of Khorne +3 points/model
  - Mark of Slaanesh +4 points/model
  - Mark of Nurgle +6 points/model
- May choose one of the following:
  - Additional hand weapons +3 points/model
  - Great weapons +9 points/model
...
- *Fell Aura:* Enemies cannot draw Line of Sight to this model unless they are within 12" of it.
  ],
)
...
#unit("VARANGUARD",
  first: true,
  profiles: (
    (name: "Varanguard", m: 4, ws: 6, bs: 3, s: 5, t: 4, w: 1, i: 5, a: 2, ld: 8, points: 68),
    (name: "Daemonic Mount", m: 8, ws: 4, bs: 0, s: 5, t: 4, w: 3, i: 3, a: 2, ld: 8, points: ""),
  ),
  unit-size: "3+",
  troop-type: "Monstrous Cavalry (Dragon Ogre)",
  base-size: "40x40, 50x50 or 50x75",
  equipment: "Light lances, heavy armour, shield, barding",
  options: [
- May choose one of the following:
  - Mark of Khorne +3 points/model
  - Mark of Slaanesh +6 points/model
  - Mark of Tzeentch +6 points/model
...
- May upgrade one Varanguard to a Standard Bearer +10 points
  ...
  special-rules: "Chaos Armour, Daemonic (Daemonic Mount), Devastating Charge, Will of Chaos",
  order: ("unit-size", "troop-type", "base-size", "equipment", "options", "special-rules"),
)
...
#unit("WARSHRINE",
  ...
  drawn-by: "2 Shrine Bearers (Chaos Beast)",
  base-size: "50x100 or 60x100",
  equipment: "Hand weapon, light armour",
  options: [
- May choose one of the following:
  ...
  - Mark of Slaanesh +25 points
  - Mark of Tzeentch +25 points
  ],
  special-rules: "Fear, Random Attacks (D3+1) (Shrine Bearers only), Ward Save (5+), Will of Chaos",
  special-rules-body: [
- *Giver of Glory:* If a friendly model or unit is within 12" of one or more Chaos Warshrines when they roll on the Eye of the Gods table, you may roll an additional D6 and discard a single dice of your choosing.
- *Favour of the Ruinous Powers:* A Warshrine knows the Favour of Chaos prayer below. However, if the Warshrine has been dedicated to a particular Chaos God, the Shrinemaster instead prays to their patron for their favour. The Shrinemaster may attempt to use their prayer at the start of each of your turns by taking a Leadership test on their own unmodified Leadership. If passed, the prayer is answered and takes immediate effect. Each Favour can target one friendly unit within 6" and lasts until the start of your next turn. A friendly unit can only be under the effect of one Favour at a time.
  ...
  - *Favour of Tzeentch:* Warshrine with Mark of Tzeentch only. The unit gains the Ward save (6+) special rule, and may re-roll 1's when taking Ward saves.
  - *Favour of Slaanesh:* Warshrine with Mark of Slaanesh only. The unit gains Immunity (Psychology) (units with Mark of Slaanesh becomes Unbreakable) and re-rolls failed charge and pursuit distances.
  ],
  notes: [
...
#unit("GIANT SPINED CHAOS BEAST",
  profiles: (
    (name: "Giant Spined Chaos Beast", m: 7, ws: 3, bs: 0, s: 6, t: 5, w: 5, i: 3, a: 5, ld: 5, points: 220),
  ),
...
- May choose one of the following:
  ...
  upgrades: (
    ...
    rule("Beast of Tzeentch")[A Beast of Tzeentch has a Ward save (6+).],
  ),
...
#unit("WARPFIRE DRAGON",
  ...
  troop-type: "Monster (Dragon)",
  base-size: "50x100 or 60x100",
  special-rules: "Fly (7), Magical Attacks, Natural Armour (3+), Regeneration (6+)",
  special-rules-body: [
- *Warpfire Blast:* A Warpfire Dragon has a shooting attack that fires like a Stone Thrower with the following profile:

#minitable(("Range", "Strength", "Special Rules"), ("6-24\"", "4(6)", "Flaming Attacks, Magical Attacks, Multiple Wounds (D3)"))

If a misfire is rolled, the Warpfire Dragon suffers D3 Wounds with no saves allowed.

- *Warpfire Aura:* At the start each round of Close Combat, all models in base contact with a Warpfire Dragon must pass a Leadership test or suffer a Wound which Ignores Armour Saves. In addition, a Warpfire Dragon has a 5+ Ward save against Magical Attacks. Furthermore, any spells targeting them must re-roll successful casting attempts.
...
#unit("HELLCANNON",
  ...
  options: [
- The Handlers may upgrade to medium armour +3 points
  ],
...
- *Doomfire:* Doomfire is fired exactly like a stone thrower with range of 12\-60", Strength 5(10), Move or Fire, Multiple Wounds (D6) and Slow to Fire special rules. Any unit that suffers one or more casualties from Doomfire must take a Panic test with a \-1 penalty to their Leadership. Should the artillery dice roll a misfire, roll a D6 on the following table:
...
Hellcannon cannot fire for the rest of the game.")))
  ...
  order: ("troop-type", "handler", "equipment", "options", "special-rules"),
)
...
then choose another victim. Roll again on this table to see what the Giant does with it.")))
  ...
  upgrades: (
    ...
    rule("Giant of Tzeentch")[A Giant of Tzeentch has a Ward save (6+).],
  ),
  order: ("troop-type", "base-size", "equipment", "options", "special-rules", "upgrades"),
)
...
#unit("CHAOS WAR MAMMOTH",
  ...
  special-rules-body: [
- *Mammoth Attacks:* When the Chaos War Mammoth attacks, roll on the appropriate following table to determine its action. If no suitable target is in base contact, roll again on the chart until you get another result.
...
another victim. Roll again on this table to see what happens.")))
  ...
  notes: [
- A Chaos War Mammoth has a Line of Sight value of 10.
  ],
)
...
#unit("SLAUGHTERPRIEST OF KHORNE",
  first: true,
  profiles: (
    (name: "Slaughterpriest", m: 4, ws: 6, bs: 3, s: 5, t: 4, w: 2, i: 5, a: 3, ld: 8, points: 145),
  ),
  troop-type: "Infantry (Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon, light armour",
...
#unit("JUGGERNAUT OF KHORNE",
  compact: true,
  profiles: (
    (name: "Juggernaut of Khorne", m: 7, ws: 4, bs: 0, s: 5, t: 4, w: 3, i: 2, a: 3, ld: 7, points: "-"),
  ),
  troop-type: "Monstrous Beast (Daemon)",
  base-size: "50x50 or 50x75",
...
#unit("ROT BEAST OF NURGLE",
  compact: true,
  profiles: (
    (name: "Rot Beast of Nurgle", m: 7, ws: 3, bs: 0, s: 4, t: 4, w: 3, i: 2, a: 2, ld: 6, points: "-"),
  ),
  troop-type: "Monstrous Beast (Daemon)",
  base-size: "50x75",
  special-rules: "Daemon of Nurgle, Poisoned Attacks, Regeneration (5+)",
)
...
#unit("SKULLREAPERS",
  ...
  unit-size: "5+",
  troop-type: "Large Infantry (Human)",
  base-size: "30x30 or 40x40",
  equipment: "Two hand weapons, medium armour",
  options: [
- May replace hand weapons with two ensorcelled weapons +5 points/model
...
#unit("HELLSTRIDERS",
  profiles: (
    (name: "Hellstrider", m: 4, ws: 4, bs: 3, s: 3, t: 3, w: 1, i: 4, a: 1, ld: 7, points: 19),
    (name: "Steed of Slaanesh", m: 10, ws: 3, bs: 0, s: 3, t: 3, w: 1, i: 5, a: 1, ld: 7, points: ""),
  ),
  unit-size: "5+",
  troop-type: "Cavalry (Human)",
...
#chart((("Rounds won", "Bonus"), ("1", "Fuelled by Pain: The unit gains +1 Attack (not including mounts)."), ("2", "Insensible to Agony: The unit gains a 4+ Ward save."), ("3", "Intoxicating Delirium: The unit gains the Unbreakable special rule.")))
  ],
  order: ("unit-size", "troop-type", "mount", "base-size", "equipment", "options", "special-rules"),
)
...
#unit("WRATHMONGERS",
  ...
  troop-type: "Large Infantry (Human)",
  base-size: "30x30 or 40x40",
  equipment: "Wraihflails, medium armour",
  equipment-body: [
...
#unit("SKULLCRUSHERS",
  profiles: (
    (name: "Skullcrusher", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 1, i: 4, a: 2, ld: 8, points: 75),
    (name: "Juggernaut of Khorne", m: 7, ws: 4, bs: 0, s: 5, t: 4, w: 3, i: 2, a: 3, ld: 7, points: ""),
  ),
  unit-size: "3+",
  ...
  base-size: "50x75",
  equipment: "Light lance, heavy armour, shield",
  options: [
- May replace light lances with one of the following:
  - Heavy lances +2 points/model
...
#unit("ROT KNIGHTS",
  profiles: (
    (name: "Rot Knight", m: 4, ws: 5, bs: 3, s: 4, t: 5, w: 1, i: 4, a: 2, ld: 8, points: 60),
    (name: "Rot Beast of Nurgle", m: 7, ws: 3, bs: 0, s: 4, t: 5, w: 3, i: 2, a: 2, ld: 6, points: ""),
  ),
  troop-type: "Monstrous Cavalry (Special Character, Human)",
  mount: "Rot Beast of Nurgle (Chaos Beast)",
  base-size: "50x75",
...
#unit("PUSGOYLE BLIGHTLORDS",
  profiles: (
    (name: "Pusgoyle Blightlord", m: 4, ws: 6, bs: 3, s: 4, t: 5, w: 2, i: 3, a: 3, ld: 8, points: 66),
    (name: "Rot Fly of Nurgle", m: 1, ws: 3, bs: 0, s: 4, t: 5, w: 3, i: 2, a: 3, ld: 7, points: ""),
  ),
  unit-size: "2+",
  troop-type: "Cavalry (Human)",
...
#minitable(("Range", "Strength", "Special Rules"), ("12\"", "4", "Multiple Wounds (D3), Poisoned Attacks, Quick to Fire"))
  ],
  options: [
...
#unit("TOAD DRAGON",
  ...
  special-rules-body: (
    ...
    rule("Unspeakable Foulness")[This is a Breath Weapon attack. Any models that are hit must pass a Toughness test at a \-1 penalty or suffer a wound with the Multiple Wounds (D3) and Ignores Armour saves special rules.],
  ),
)

#unit("SLICKBLADE SEEKERS",
  profiles: (
    (name: "Seeker", m: 4, ws: 4, bs: 4, s: 4, t: 3, w: 1, i: 4, a: 1, ld: 8, points: 40),
  ...
  options: [
- May replace polearms & Killing Blow with bows (with Poisoned Attacks) and the Quick to Fire special rule +2 points/model
...
#unit("FATEMASTERS",
  profiles: (
    (name: "Fatemaster", m: 4, ws: 5, bs: 3, s: 4, t: 4, w: 2, i: 4, a: 2, ld: 8, points: 60),
    (name: "Disc of Tzeentch", m: 0, ws: 3, bs: 0, s: 4, t: 4, w: 1, i: 4, a: 2, ld: 7, points: ""),
  ),
  unit-size: "3+",
  troop-type: "Cavalry (Human)",
  ...
  magic: "A unit of Fatemasters is a Level 1 Wizard that can use spells from the Lore of Tzeentch.",
  options: [
...
#unit("ARCHAON THE EVERCHOSEN",
  ...
  profiles: (
    (name: "Archaon the Everchosen", m: 4, ws: 9, bs: 5, s: 5, t: 5, w: 4, i: 7, a: 5, ld: 10, points: 580),
    (name: "Dorghar (Daemonic Mount)", m: 8, ws: 4, bs: 0, s: 5, t: 5, w: 3, i: 3, a: 3, ld: 9, points: ""),
  ),
  ...
  magic-items: (
    ...
    rule("The Eye of Sheerian")[Talisman. The Eye of Sheerian grants Archaon a Ward save (3+).],
...
- May be mounted on Dorghar +50 points
  ...
  special-rules-body: (
    rule("Chosen of the Gods")[As the Chosen of Gods, Archaon has the following bonuses: Immunity (Psychology), Immunity (Poisoned Attacks), Magic Resistance (2), and he may re-roll any channelling dice rolls of a 1. Re- roll any Eye of the Gods rolls of 2 and 12.],
...
#unit("COUNT MORDREK",
  ...
  special-rules: "Chaos Armour, Eye of the Gods, Will of Chaos",
  special-rules-body: (
    rule("Living Damnation")[At the start of each of his turns, Mordrek must roll a D6 for his Weapon Skill and Attacks characteristics, as well a D3 for his Strength and Toughness and characteristics, and add the result to his profile.],
  ),
)
...
#unit("HARALD HAMMERSTORM",
  ...
  profiles: (
    (name: "Harald Hammerstorm", m: 4, ws: 8, bs: 3, s: 5, t: 5, w: 3, i: 8, a: 5, ld: 9, points: 250),
  ),
  troop-type: "Infantry (Special Character, Human)",
...
with this weapon, the enemy unit suffers D3 Wounds with no saves allowed.")))

- *Warpsteel Shield:* Magic Armour. Shield. This shield gives the bearer a 4+ Ward save against non-magical attacks.
  ],
  special-rules: "Chaos Armour, Eye of the Gods, Impact Hits (D3), Natural Armour (5+), Will of Chaos",
  special-rules-body: (
    rule("Warlord of the First Circle")[Friendly units of Varanguard wihin 12" of Abraxia may re-roll failed Leadership tests. In addition, you may take one unit of Varanguard as a Special Unit instead of a Rare Unit.],
...
#unit("SAYL THE FAITHLESS",
  ...
  profiles: (
    (name: "Sayl the Faithless", m: 4, ws: 6, bs: 3, s: 4, t: 4, w: 3, i: 6, a: 3, ld: 9, points: 375),
    (name: "Nightmaw (Chaos Spawn)", m: "*", ws: 3, bs: 0, s: 4, t: 5, w: 3, i: 2, a: "*", ld: 10, points: ""),
  ),
  troop-type: "Infantry (Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Light armour.",
  magic: "Sayl is a Level 3 Wizard who uses spells from the Lore of the Heavens or Lore of Shadow.",
  magic-items: (
    rule("The Viperous Staff")[Magic Weapon. All attacks with the Viperous Staff have the Killing Blow special rule. In addition, it grants the wielder a 5+ Ward save.],
  ...
  special-rules-body: (
    ...
    rule("Traitor's Mist")[Innate Bound Spell, power level 4. _Traitor's Mist_ is an *augment* spell that Sayl can cast upon himself. Sayl may immediately make a move using the Fly (10) special rule as if it were the Remaining Moves sub-phase. In addition, any model (friend or foe) in base contact at the start of the move suffers an automatic Strength 3 hit which Ignores Armour saves.],
  ),
)
...
#unit("MARAKARR BLOOD-SKY",
  ...
  profiles: (
    (name: "Marakarr Blood-Sky", m: 4, ws: 7, bs: 3, s: 4, t: 4, w: 3, i: 6, a: 4, ld: 9, points: 185),
  ),
  troop-type: "Infantry (Special Character, Human)",
  base-size: "25x25",
  equipment: "Hand weapon, light armour",
  magic-items: (
    rule("Infernal Runeshield")[Shield. This shield gives Marakarr a 5+ Ward save. In addition, whenever she makes a successful Parry save, the model that struck the blow suffer one Wound which Ignores Armour Saves.],
  ),
  options: [
- May take Magic Items up to a total of 50 points
  ],
...
#unit("KHOLEK SUNEATER",
  ...
  special-rules: "Immunity (Lightning Attacks, Psychology), Impact Hits (D6), Natural Armour (4+), Storm",
  special-rules-body: [
*Call* (see Dragon Ogre Shaggoth)
...
#unit("WULFRIK THE WANDERER",
  ...
  profiles: (
    (name: "Wulfrik the Wanderer", m: 4, ws: 8, bs: 3, s: 5, t: 4, w: 2, i: 7, a: 4, ld: 8, points: 185),
  ),
  troop-type: "Infantry (Special Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Hand weapon, heavy armour, shield",
  ...
  special-rules: "Chaos Armour, Eye of the Gods, Will of Chaos",
  special-rules-body: (
    rule("Gift of Tongues")[When Wulfrik the Wanderer issues a challenge, your opponent cannot choose to refuse it. Furthermore, Wulfrik nominates which enemy character will accept the challenge.],
    rule("Hunter of Champions")[As soon as Wulfrik the Wanderer is placed on the tabletop, nominate one enemy character in your opponent's army – this is the champion Wulfrik has journeyed to challenge. Against the chosen character, Wulfrik always Hit and Wound on a 2+ and gets the Killing Blow special rule.],
...
#unit("ATARUS",
  ...
  profiles: (
    (name: "Atarus", m: 4, ws: 8, bs: 3, s: 5, t: 4, w: 2, i: 7, a: 4, ld: 9, points: 190),
    (name: "Drakocephalus (Daemonic Mount)", m: 8, ws: 4, bs: 0, s: 5, t: 4, w: 3, i: 3, a: 2, ld: 7, points: ""),
  ),
  troop-type: "Monstrous Cavalry (Special Character, Human)",
...
#unit("THROGG WINTERTOOTH",
  ...
  profiles: (
    (name: "Throgg", m: 6, ws: 5, bs: 2, s: 6, t: 5, w: 4, i: 2, a: 5, ld: 8, points: 265),
  ),
  ...
  special-rules: "Eye of the Gods, Natural Armour (6+)",
  special-rules-body: (
    rule("Copious Vomit")[Throgg has a Strength 5 Breath Weapon with the Ignores Armour saves special rule.],
    rule("Lord of the Monstrous Horde")[If your army includes Throgg, units of Chaos Trolls count as Core choices instead of Special choices, and at least one unit must be included.],
    rule("Mutant Regeneration")[Throgg has the Regeneration (4+) special rule. In addition, if Throgg Regenerates two or more Wounds in the same phase, he rolls on the Eye of the Gods table at the end of that phase.],
  ),
)

= MARKED SPECIAL CHARACTERS (LORDS)

#unit("VALKIA THE BLOODY",
  first: true,
  subtitle: "Bringer of Glory, the Gorequeen, Swordmaiden of the Blood God",
  profiles: (
    (name: "Valkia the Bloody", m: 4, ws: 9, bs: 3, s: 5, t: 5, w: 3, i: 8, a: 5, ld: 9, points: 350),
  ),
  troop-type: "Infantry (Special Character, Human)",
  base-size: "25x25 or 30x30",
  magic-items: (
  ...
  special-rules: "Chaos Armour, Eye of the Gods, Fear, Fly (10), Mark of Khorne, Ward save (5+), Will of Chaos",
  special-rules-body: (
    rule("Consort of the Blood God")[If Valkia the Bloody is required to roll on the Eye of the Gods table, do not roll any dice. Instead, she is always counted as having rolled the Slaughterer's Strength reward.],
    rule("The Gaze of Khorne")[All friendly units with the Mark of Khorne within 12" of Valkia the Bloody re-roll failed Break tests. However, any friendly unit with the Mark of Khorne that flees whilst within 12" of Valkia the Bloody, for whatever reason, immediately suffers D6 Strength 6 hits.],
  ),
)

#unit("ARBAAL THE DESTROYER",
  subtitle: "Destroyer of Khorne",
  profiles: (
    (name: "Arbaal", m: 4, ws: 8, bs: 3, s: 5, t: 5, w: 3, i: 8, a: 5, ld: 9, points: 300),
  ...
  special-rules-body: (
    ...
    rule("Wrath of Khorne")[Arbaal must always issue and accept challenges. In addition, if Arbaal flees for any reason, he will be turned into Chaos Spawn of Khorne. Make the flee move with Arbaal as normal, and then replace the model with a Chaos Spawn of Khorne following the rules for Damned by Chaos on the Eye of the Gods table.],
  ),
)
...
#unit("SKARR BLOODWRATH",
  ...
  troop-type: "Large Infantry (Special Character, Human)",
  base-size: "30x30 or 40x40",
  equipment: "Light armour",
  magic-items: (
...
#unit("VORGAROTH THE SCARRED",
  ...
  profiles: (
    (name: "Vorgaroth", m: 4, ws: 8, bs: 3, s: 5, t: 5, w: 3, i: 7, a: 5, ld: 9, points: 790),
...
#unit("TAMURKHAN",
  ...
  magic-items: (
    ...
    rule("The Black Cleaver")[Magic Weapon. Tyrant only. Great weapon. Any model that suffers a wound from the weapon and survives must take a Toughness test or suffer a permanent reduction of \-1 to their Toughness score.],
  ),
  options: [
- May be mounted on Bubebolos +375 points
  ],
  special-rules: "Chaos Armour, Eye of the Gods, Mark of Nurgle (included in profile), Unbreakable",
  special-rules-body: [
...
#unit("THE GLOTTKIN",
  ...
  profiles: (
    (name: "Otto Glott", m: 4, ws: 8, bs: 3, s: 5, t: "-", w: "-", i: 6, a: 5, ld: 9, points: 850),
    (name: "Ethrac Glott", m: 4, ws: 5, bs: 3, s: 5, t: "-", w: "-", i: 5, a: 3, ld: 8, points: ""),
...
#unit("ORGHOTTS DAEMONSPEW",
  subtitle: "Master of the Pox Maggoths",
  profiles: (
    (name: "Orghotts Daemonspew", m: "-", ws: 8, bs: 3, s: 5, t: 6, w: 3, i: 7, a: 5, ld: 9, points: 420),
    (name: "Whippermaw", m: 6, ws: 3, bs: 3, s: 6, t: 6, w: 6, i: 1, a: 4, ld: "-", points: ""),
  ),
  troop-type: "Monster (Special Character, Human)",
  mount: "Whippermaw (Chaos Beast)",
  base-size: "60x100",
  equipment: "Heavy armour",
  magic-items: (
    rule("The Rotaxes")[Magic Weapon. Two hand weapons. Close combat attacks made by the wielder have +2 Strength and the Poisoned Attacks special rule.],
  ),
  special-rules: "Chaos Armour, Eye of the Gods, Mark of Nurgle (included in profile), Ward Save (6+), Will",
  special-rules-body: [
*of Chaos*
...
#minitable(("Range", "Strength", "Special Rules"), ("6\"", "4", "Killing Blow, Poisoned Attacks, Quick to Fire"))
  ],
)

#unit("BLOAB ROTSPAWNED",
  subtitle: "Harbinger of Fecundity",
  profiles: (
    (name: "Bloab Rotspawned", m: "-", ws: 5, bs: 3, s: 4, t: 5, w: 3, i: 5, a: 3, ld: 8, points: 460),
    (name: "Bilespurter", m: 6, ws: 3, bs: 3, s: 6, t: 6, w: 6, i: 1, a: 4, ld: "-", points: ""),
  ),
  troop-type: "Monster (Special Character, Human)",
  mount: "Bilespurter (Chaos Beast)",
  base-size: "60x100",
  equipment: "Great weapon, medium armour",
  ...
  special-rules-body: [
- *Bilespurter:* Bloab Rotspawned can make a shooting attack in the same manner as a stone thrower, using the following profile.

#minitable(("Range", "Strength", "Special Rules"), ("24\"", "3(4)", "Ignores Armour Saves"))
...
#unit("MORBIDEX TWICEBORN",
  subtitle: "Enemy of Sterility",
  profiles: (
    (name: "Morbidex Twiceborn", m: "-", ws: 7, bs: 3, s: 5, t: 6, w: 3, i: 6, a: 4, ld: 8, points: 415),
    (name: "Tripletongue", m: 6, ws: 3, bs: 3, s: 6, t: 6, w: 6, i: 1, a: 4, ld: "-", points: ""),
  ),
  troop-type: "Monster (Special Character, Human)",
  mount: "Tripletongue (Chaos Beast)",
  base-size: "60x100",
  equipment: "Great weapon, medium armour",
  mutations-and-powers: "Nurgle's Rot",
  special-rules: "Chaos Armour, Eye of the Gods, Mark of Nurgle (included in profile), Regeneration (4+), Will of Chaos",
  special-rules-body: [
...
#minitable(("Range", "Strength", "Special Rules"), ("6\"", "6", "Poisoned Attacks, Quick To Fire"))
  ],
)
...
- May take Magic Items up to a total of 50 points
  ...
  order: ("troop-type", "base-size", "equipment", "options", "mutations-and-powers", "special-rules"),
)
...
#unit("SIGVALD THE MAGNIFICENT",
  ...
  mutations-and-powers: "Allure of Slaanesh, Diabolic Splendour",
  special-rules: "Chaos Armour, Eye of the Gods, Mark of Slaanesh, Strider, Stupidity",
  special-rules-body: (
    rule("Favoured Son")[If Sigvald the Magnificent is required to roll on the Eye of the Gods table, do not roll any dice. Instead, he is always counted as having rolled the Dark Fury reward.],
...
#unit("GLUTOS ORSCOLLION",
  ...
  profiles: (
    (name: "Glutos Orscollion", m: "-", ws: 5, bs: 3, s: 5, t: "-", w: "-", i: 3, a: 3, ld: 9, points: 675),
  ...
  magic: "Glutos Orscollion is a level 4 Wizard who uses spells from the Lore of Slaanesh.",
  magic-items: (
    rule("The Leerstave of Loth'shar")[Arcane Item. The Leerstave gives Glutos a +1 casting and dispelling bonus.],
  ),
  special-rules: "Eye of the Gods, Mark of Slaanesh, Ward Save (5+).",
  special-rules-body: [
- *Crippling Famishment:* Innate Bound Spell, power level 5+. _Crippling Famishment_ is a *hex* spell with a range of 18". The target unit must divide all its movement by half (rounding up) until the start of the caster's next Magic phase.
...
#unit("VILITCH THE CURSELING",
  ...
  profiles: (
    (name: "Vilitch the Curseling", m: 4, ws: 5, bs: 3, s: 5, t: 4, w: 3, i: 5, a: 3, ld: 8, points: 360),
  ),
  troop-type: "Infantry (Special Character, Human)",
  base-size: "25x25 or 30x30",
  equipment: "Heavy armour",
  ...
  mutations-and-powers: "Conjoined Homunculus",
...
#unit("GALRAUCH",
  ...
  profiles: (
    (name: "Galrauch", m: 6, ws: 6, bs: 0, s: 6, t: 6, w: 6, i: 6, a: 6, ld: 9, points: 525),
  ),
  troop-type: "Monster (Special Character, Dragon)",
  base-size: "50x50, 50x100, 60x100",
  magic: "Galrauch is a Level 4 Wizard who uses spells from the Lore of Tzeentch.",
  special-rules: "Dark Fire of Chaos (see Chaos Dragons), Fly (7), Mark of Tzeentch, Natural Armour (3+), Ward Save (6+)",
  special-rules-body: (
...
= MARKED SPECIAL CHARACTERS (HEROES)

#unit("SCYLA ANFINGRIMM",
  first: true,
  subtitle: "The Bloodbeast, Scourgeborn, the Talon of Khorne",
  profiles: (
    (name: "Scyla Anfingrim", m: 6, ws: 4, bs: 0, s: 5, t: 5, w: 4, i: 3, a: "*", ld: 10, points: 105),
  ),
  troop-type: "Monstrous Beast (Special Character, Chaos Beast)",
  base-size: "40x40 or 50x50",
  magic-items: (
    rule("Brass Collar of Khorne")[Talisman. The Brass Collar of Khorne bestows Scyla Anfingrimm with a 6+ Ward save and the Magic Resistance (3) special rule.],
  ),
...
#unit("KAYZK THE BEFOULED",
  profiles: (
    ...
    (name: "Rotbeast of Nurgle", m: 7, ws: 3, bs: 0, s: 4, t: 4, w: 3, i: 2, a: 2, ld: 6, points: ""),
  ),
  troop-type: "Monstrous Cavalry (Special Character, Human)",
  mount: "Rotbeast of Nurgle (Chaos Beast)",
  base-size: "50x75",
  equipment: "Heavy armour",
  magic-items: (
...
#chart((("D6", "Result"), ("1-3", "The Red Plague: The unit suffers D6 Strength 3 hits which Ignores Armour saves."), ("4-5", "Brain Fever: The unit suffers -2 to their Leadership until the start of the next round."), ("6", "Black Rot: All the members of the unit suffer a -1 Toughness penalty until the start of the
next round.")))
  ],
)
...
#unit("FESTUS THE LEECHLORD",
  ...
  special-rules: "of the Gods, Mark of Nurgle (included in profile), Poisoned Attacks, Regeneration (4+), Will",
  special-rules-body: [
*of Chaos*
...
- *Dark Experiments:* When making a pursuit move, Festus and any unit he is with will only pursue 1D6" as they begin to bind their captives. However, enemy units caught by Festus or his unit are worth double victory points.
  ],
)
...
#unit("DECHALA",
  ...
  special-rules: "Chaos Armour, Always Strikes First, Eye of the Gods, Mark of Slaanesh, Poisoned Attacks.",
  special-rules-body: [
- *Dances of Slaanesh:* Dechala may pick one of the following dances at the start of each round of close combat, the effect of which lasts until the end of that turn or until a new dance is chosen. She cannot choose the same dance in two consecutive rounds of combat.
  ...
  - *Dance of Destruction:* Dechala gains +1 To Hit.
