...
The *Ordo Draconis* is the army this book is named after and cannot field: a vampire aristocracy that fights in its own person. Its knights ride and its household walks, its lord is a feudal protector rather than a butcher, and the dead are still raised — as fodder, and no longer as the backbone. It moves two entries up one category each, adds three new entries, takes all missile weapons off the table, unbinds the mounts from the grave, and lets nothing but vampires count towards what an army must spend on Core.
...
- *Only units with the Vampire keyword count towards the minimum points an army must spend on Core units.* Every other Core entry may be taken exactly as the book allows — the Undead, the Crypt Ghouls and the bats alike — and buys you nothing towards the quarter. Take as many as you like.
...
#chart((
  ...
  ("Skeletons, Zombies, Wolves, Spirit Hosts, Crypt Ghouls, both kinds of bat", "Core, and count towards the 25% minimum", "Core, and count towards nothing"),
...
The Core-minimum clause is the cleanest thing here, and it went through a draft that did not work. It read *Undead* where it now reads *Vampire*, and the Core rabble in this book is not all Undead: Crypt Ghouls are Infantry (Ghoul) at 7 points with Toughness 4, two Attacks, Fear and Poisoned Attacks, and the Fell Bats and Bat Swarms are Chiropters. None of the three is Undead and none is Human, so all three counted in full — two ghoul blocks and a flight of bats cleared the 500 points a 2,000-point army owes without a single vampire in it, and the clause the whole list turns on bought nothing. Reading it off the *Vampire* keyword closes that, and the objection it invites is now the opposite one: the minimum can only be met from Vargheists, Blood Guard and Blood Knights, so the shape of a Legion's Core is prescribed even though nothing is banned, and the first Blood Dragon's licence is doing real work at every points value rather than offering an option.
